...
def compareConvection():
    """Compare different CO2 phase distributions in Box B for Case B of the 11th SPE CSP"""
    ...
    for i, group in zip(range(len(groups)), groups):
        ...
        if len(csvData[0]) > 7:
            axsM[0].plot(t, 1e-6*csvData[:, 7], label=group, color=color, linestyle=ls)
        if len(csvData[0]) > 8:
            axsI[0].plot(t, 1e-6*csvData[:, 8], label=group, color=color, linestyle=ls)
        if len(csvData[0]) > 9:
            axsD[0].plot(t, 1e-6*csvData[:, 9], label=group, color=color, linestyle=ls)
        if len(csvData[0]) > 10:
            axsS[0].plot(t, 1e-6*csvData[:, 10], label=group, color=color, linestyle=ls)
