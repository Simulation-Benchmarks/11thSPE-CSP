...
def visualizeSpatialMaps():
    """Visualize spatial maps for Case C of the SPE11 CSP"""
    ...
    for i, group in zip(range(len(groups)), groups):
        if group[-2] != '-':
            fileName = os.path.join(folder, group.lower(), f'spe11c_spatial_map_{time}y.csv')
        else:
            fileName = os.path.join(folder, group[:-2].lower(), f'result{group[-1]}', f'spe11c_spatial_map_{time}y.csv')

        p, s, mCO2, mH2O, rhoG, rhoL, tmCO2, temp = getFieldValues(fileName, nX, nY)

        if len(groups) == 1:
            # scale pressure to bars
            plotColorMesh(fig, x, y, 1e-5*p, 0, "pressure [bar]", 200, 350)
            ...
            plotColorMesh(fig, x, y, 1e3*mH2O, 3, "H2O mass frac in gas [g/kg]", 1, 4)
            plotColorMesh(fig, x, y, rhoG, 4, "gas phase density [kg/m3]", 0.8e3, 1.0e3)
            plotColorMesh(fig, x, y, rhoL, 5, "liquid phase density [kg/m3]", 0.99e3, 1.03e3)
            # scale mass to kilotons
            plotColorMesh(fig, x, y, 1e-6*tmCO2, 6, "total CO2 mass [kt]", 0, 4.5)
            plotColorMesh(fig, x, y, temp, 7, "temperature [°C]", 20, 70)
        else:
            # scale pressure to bars
            plotColorMesh(figP, x, y, 1e-5*p, i, group, 200, 350)
            plotColorMesh(figS, x, y, s, i, group, 0, 1)
            # scale mass fractions to g/kg
            plotColorMesh(figMCO2, x, y, 1e3*mCO2, i, group, 0, 70)
            plotColorMesh(figMH2O, x, y, 1e3*mH2O, i, group, 1, 4)
            plotColorMesh(figRhoG, x, y, rhoG, i, group, 0.8e3, 1.0e3)
            plotColorMesh(figRhoL, x, y, rhoL, i, group, 0.99e3, 1.03e3)
            # scale mass to kilotons
            plotColorMesh(figTmCO2, x, y, 1e-6*tmCO2, i, group, 0, 4.5)
            plotColorMesh(figTemp, x, y, temp, i, group, 20, 70)
