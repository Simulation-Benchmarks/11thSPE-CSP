...
def compareConvection():
    """Compare different CO2 phase distributions in Box A for Case C of the 11th SPE CSP"""
    ...
    mobileFromSpatialMapsFileName = os.path.join(tableFolder, 'spe11c_mobile_boxA_from_spatial_maps.csv')
    ...
    immobileFromSpatialMapsFileName = os.path.join(tableFolder, 'spe11c_immobile_boxA_from_spatial_maps.csv')
    immobileFromSpatialMaps = np.genfromtxt(immobileFromSpatialMapsFileName, delimiter=',', names=True)
    dissolvedFromSpatialMapsFileName = os.path.join(tableFolder, 'spe11c_dissolved_boxA_from_spatial_maps.csv')
    dissolvedFromSpatialMaps = np.genfromtxt(dissolvedFromSpatialMapsFileName, delimiter=',', names=True)
    sealFromSpatialMapsFileName = os.path.join(tableFolder, 'spe11c_seal_boxA_from_spatial_maps.csv')
    ...
    for i, group in zip(range(len(groups)), groups):
        ...
        if len(csvData[0]) > 3:
            axsM[0].plot(t, 1e-9*csvData[:, 3], label=group, color=color, linestyle=ls)
        if len(csvData[0]) > 4:
            axsI[0].plot(t, 1e-9*csvData[:, 4], label=group, color=color, linestyle=ls)
        if len(csvData[0]) > 5:
            axsD[0].plot(t, 1e-9*csvData[:, 5], label=group, color=color, linestyle=ls)
        if len(csvData[0]) > 6:
            axsS[0].plot(t, 1e-9*csvData[:, 6], label=group, color=color, linestyle=ls)

        columnName = group.replace('-', '')
        axsM[1].plot(tSpatialMaps, 1e-9*mobileFromSpatialMaps[columnName], label=group, color=color, linestyle=ls)
        axsI[1].plot(tSpatialMaps, 1e-9*immobileFromSpatialMaps[columnName], label=group, color=color, linestyle=ls)
        axsD[1].plot(tSpatialMaps, 1e-9*dissolvedFromSpatialMaps[columnName], label=group, color=color, linestyle=ls)
        axsS[1].plot(tSpatialMaps, 1e-9*sealFromSpatialMaps[columnName], label=group, color=color, linestyle=ls)

    axsM[0].set_title(r'reported')
    axsM[0].set_ylabel(r'mass [Mt]')
    axsM[0].set_xscale(r'log')
    axsM[0].set_xlim((1e0, 1e3))
    axsM[0].set_ylim((-1e0, 65))
    axsM[1].set_title(r'from spatial maps')
    axsM[1].set_ylabel(r'mass [Mt]')
    axsM[1].set_xscale(r'log')
    axsM[1].set_xlim((1e0, 1e3))
    axsM[1].set_ylim((-1e0, 65))
    axsM[1].yaxis.tick_right()
    axsM[1].yaxis.set_label_position('right')
    figM.suptitle(r'Box A: mobile gaseous CO$_2$')
    handles, labels = axsM[1].get_legend_handles_labels()
    figM.legend(handles, labels, loc='center left', bbox_to_anchor=(1, 0.5), ncol=2)
    figM.tight_layout()
    figM.savefig('spe11c_compare_mobA.png', bbox_inches='tight', dpi=300)

    axsI[0].set_title(r'reported')
    axsI[0].set_ylabel(r'mass [Mt]')
    axsI[0].set_xscale(r'log')
    axsI[0].set_xlim((1e0, 1e3))
    axsI[0].set_ylim((-1e-2, 1.1e0))
    axsI[1].set_title(r'from spatial maps')
    axsI[1].set_ylabel(r'mass [Mt]')
    axsI[1].set_xscale(r'log')
    axsI[1].set_xlim((1e0, 1e3))
    axsI[1].set_ylim((-1e-2, 1.1e0))
    axsI[1].yaxis.tick_right()
    axsI[1].yaxis.set_label_position('right')
    figI.suptitle(r'Box A: immobile gaseous CO$_2$')
    handles, labels = axsI[1].get_legend_handles_labels()
    figI.legend(handles, labels, loc='center left', bbox_to_anchor=(1, 0.5), ncol=2)
    figI.tight_layout()
    figI.savefig('spe11c_compare_immA.png', bbox_inches='tight', dpi=300)

    axsD[0].set_title(r'reported')
    axsD[0].set_ylabel(r'mass [Mt]')
    axsD[0].set_xscale(r'log')
    axsD[0].set_xlim((1e0, 1e3))
    axsD[0].set_ylim((-5e-1, 3.0e1))
    axsD[1].set_title(r'from spatial maps')
    axsD[1].set_ylabel(r'mass [Mt]')
    axsD[1].set_xscale(r'log')
    axsD[1].set_xlim((1e0, 1e3))
    axsD[1].set_ylim((-5e-1, 3.0e1))
    axsD[1].yaxis.tick_right()
    axsD[1].yaxis.set_label_position('right')
    figD.suptitle(r'Box A: dissolved CO$_2$')
    handles, labels = axsD[1].get_legend_handles_labels()
    figD.legend(handles, labels, loc='center left', bbox_to_anchor=(1, 0.5), ncol=2)
    figD.tight_layout()
    figD.savefig('spe11c_compare_dissA.png', bbox_inches='tight', dpi=300)

    axsS[0].set_title(r'reported')
    axsS[0].set_ylabel(r'mass [Mt]')
    axsS[0].set_xscale(r'log')
    axsS[0].set_xlim((1e0, 1e3))
    axsS[0].set_ylim((-5e-2, 1.0e0))
    axsS[1].set_title(r'from spatial maps')
    axsS[1].set_ylabel(r'mass [Mt]')
