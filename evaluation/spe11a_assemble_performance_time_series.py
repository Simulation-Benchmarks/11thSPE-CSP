...
def assembleTimeSeries():
    ...
    for group in groups:
        if group[-2] != '-':
            if detailed == "False":
                fileName = f'{folder}/{group.lower()}/spe11a_performance_time_series.csv'
            else:
                fileName = f'{folder}/{group.lower()}/spe11a_performance_time_series_detailed.csv'
            color = groups_and_colors[group.lower()]
            ls = '-'
        else:
            if detailed == "False":
                fileName = f'{folder}/{group[:-2].lower()}/result{group[-1]}/spe11a_performance_time_series.csv'
            else:
                fileName = f'{folder}/{group[:-2].lower()}/result{group[-1]}/spe11a_performance_time_series_detailed.csv'
