...
def calculateL2Differences():
    ...
    for i, groupI in zip(range(numGroups), groups):
        ...
        if groupI[-2] != '-':
            if not groupFolders:
                baseFolderI = os.path.join(folder, groupI.lower(), 'spe11a')
        else:
            if not groupFolders:
                baseFolderI = os.path.join(folder, groupI[:-2].lower(), 'spe11a', f'result{groupI[-1]}')
        ...
        for j, groupJ in zip(range(numGroups), groups):
            ...
            if groupJ[-2] != '-':
                if not groupFolders:
                    baseFolderJ = os.path.join(folder, groupJ.lower(), 'spe11a')
            else:
                if not groupFolders:
                    baseFolderJ = os.path.join(folder, groupJ[:-2].lower(), 'spe11a', f'result{groupJ[-1]}')
