...
def calculateCO2Distribution():
    ...
    for i, group in zip(range(numGroups), groups):
        ...
        fileName = os.path.join(baseFolder, f'spe11a_spatial_map_0h.csv')
        p, s, mCO2, mH2O, rhoG, rhoL, tmCO2 = getFieldValues(fileName, nX, nY)
        mCO2InBoxA0 = np.nan_to_num(mCO2[0:60, 109:280])
        sInBoxA0 = np.nan_to_num(s[0:60, 109:280])
        nXBoxA = len(mCO2InBoxA0[0])
        nYBoxA = len(mCO2InBoxA0)
        mCO2InBoxB0 = np.nan_to_num(mCO2[59:120, 0:110])
        sInBoxB0 = np.nan_to_num(s[59:120, 0:110])
        ...
        for hour in range(0, numTimeSteps*dt+1, dt):
            ...
            p, s, mCO2, mH2O, rhoG, rhoL, tmCO2 = getFieldValues(fileName, nX, nY)
            mCO2InBoxA = np.nan_to_num(mCO2[0:60, 109:280]) - mCO2InBoxA0
            ...
            sInBoxA = np.nan_to_num(s[0:60, 109:280]) - sInBoxA0
            sInBoxA[sInBoxA < 0] = 0
            mCO2InBoxB = np.nan_to_num(mCO2[59:120, 0:110]) - mCO2InBoxB0
            ...
            sInBoxB = np.nan_to_num(s[59:120, 0:110]) - sInBoxB0
            sInBoxB[sInBoxB < 0] = 0
