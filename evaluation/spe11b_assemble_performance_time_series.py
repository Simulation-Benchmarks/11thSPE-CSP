...
def assembleTimeSeries():
    ...
    for group in groups:
        if group[-2] != '-':
            if detailed == "False":
                fileName = os.path.join(folder, group.lower(), 'spe11b_performance_time_series.csv')
            else:
                fileName = os.path.join(folder, group.lower(), 'spe11b_performance_time_series_detailed.csv')
            color = groups_and_colors[group.lower()]
            ls = '-'
        else:
            if detailed == "False":
                fileName = os.path.join(folder, group[:-2].lower(), f'result{group[-1]}', 'spe11b_performance_time_series.csv')
            else:
                fileName = os.path.join(folder, group[:-2].lower(), f'result{group[-1]}', 'spe11b_performance_time_series_detailed.csv')
