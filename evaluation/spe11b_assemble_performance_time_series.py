...
def assemblePerformanceTimeSeries():
    ...
    for i, group in zip(range(len(groups)), groups):
        ...
        if group in detailed:
            ...
            fileName = os.path.join(baseFolder, 'spe11b_performance_time_series.csv')
        ...
        axsM.plot(t, csvData[:, 3], label=label, color=color, linestyle=ls)
    ...
    axsT.set_ylim([1e4, 2e8])
    utils.add_legend(axsT)
    ...
    axsRT[1].yaxis.tick_right()
    axsRT[1].yaxis.set_label_position('right')
    utils.add_legend(axsRT[0])
    ...
    figM.savefig(f'spe11b_time_series_mass.png', bbox_inches='tight', dpi=300)
    figD.savefig(f'spe11b_time_series_dof.png', bbox_inches='tight', dpi=300)
    figN.savefig(f'spe11b_time_series_nliter.png', bbox_inches='tight', dpi=300)
    figR.savefig(f'spe11b_time_series_nres.png', bbox_inches='tight', dpi=300)
    figL.savefig(f'spe11b_time_series_liniter.png', bbox_inches='tight', dpi=300)
    figRT.savefig(f'spe11b_time_series_runtime.png', bbox_inches='tight', dpi=300)
    figPub.savefig(f'spe11b_performance_time_series.png', bbox_inches='tight', dpi=300)
