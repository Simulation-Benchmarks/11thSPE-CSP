...
def assembleTimeSeries():
    ...
    for group in groups:
        if group[-2] != '-':
            fileName = os.path.join(folder, group.lower(), 'spe11c_time_series.csv')
            color = groups_and_colors[group.lower()]
            ls = '-'
        else:
            fileName = os.path.join(folder, group[:-2].lower(), f'result{group[-1]}', 'spe11c_time_series.csv')
